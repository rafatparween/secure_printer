...
export default function PricingPlans() {
  ...
  const plans = [
    ...
    },
    {
      ...
      subtitle: "Norton 360 with LifeLock",
      bgColor: "",
      price: {
        save: "Save 47%*",
        original: "$189.99",
        current: "$99.99 first year",
        monthly: "$8.33"
      }
    }
  ];
  ...
  return (
    <div className="w-full bg-[#f3f4f6] py-8 sm:py-12 md:py-16 px-4 overflow-x-hidden">
      {/* Heading */}
      <div className="max-w-[1256px] mx-auto text-center mb-8 sm:mb-10 md:mb-12">
      ...
      {/* Mobile Plan Cards - Visible only on small screens */}
      <div className="block lg:hidden max-w-[600px] mx-auto mb-6 space-y-4">
        {plans.map((plan, index) => (
          <div key={index} className={`rounded-xl border border-gray-200 overflow-hidden ${plan.bgColor}`}>
            <div className="p-6">
              {plan.tag && (
                <span className="bg-yellow-400 text-gray-900 text-xs px-3 py-1.5 rounded-full font-medium inline-block">
                  {plan.tag}
                </span>
              )}
              <h3 className="mt-3 text-gray-500 text-sm">{plan.subtitle}</h3>
              <h2 className="text-2xl font-bold mt-1">{plan.name}</h2>
              <p className="text-green-600 mt-3 font-semibold">{plan.price.save}</p>
              <p className="text-sm text-gray-400 line-through">{plan.price.original}</p>
              <p className="text-sm font-medium text-gray-700">{plan.price.current}</p>
              <p className="mt-3 text-sm text-gray-500">It works out as</p>
              <p className="text-3xl font-bold mt-1 text-gray-900">
                {plan.price.monthly} <span className="text-base font-normal text-gray-500">/month</span>
              </p>
              <button className={`w-full mt-6 px-6 py-3.5 rounded-lg font-semibold transition-all hover:shadow-md bg-yellow-400 hover:bg-yellow-500 text-gray-900`}>
                Buy {plan.name}
              </button>
              <p className="text-xs text-gray-500 mt-4 text-center">Subscription details below.*</p>
            </div>
          </div>
        ))}
      </div>

      {/* Desktop Table - Hidden on mobile, visible on lg screens and above */}
      <div className="hidden lg:block max-w-[1256px] mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
        {/* Plans Header */}
        <div className="grid grid-cols-4 border-b border-gray-200">
          <div className="p-8 font-semibold text-xl text-gray-900">
            Compare plans
          ...
          {/* Deluxe */}
          <div className="p-8 border-l border-gray-200 bg-gray-50">
            <h3 className="mt-4 text-gray-500 text-sm">Norton 360</h3>
            <h2 className="text-3xl font-bold mt-1">Deluxe</h2>
            <p className="text-green-600 mt-4 font-semibold text-lg">Save 58%*</p>
            <p className="text-sm text-gray-400 line-through">$119.99</p>
        ...
        <div className="grid grid-cols-4 border-b border-gray-200">
          ...
          {["Standard", "Deluxe", "Select Plus"].map((plan, i) => (
            <div key={i} className={`p-6 border-l border-gray-200 ${i === 1 ? "bg-gray-50" : ""}`}>
              <p className="text-sm text-gray-500 mb-4">Subscription details below.*</p>
              <button className={`w-full px-6 py-3.5 rounded-lg font-semibold transition-all hover:shadow-md bg-yellow-400 hover:bg-yellow-500 text-gray-900`}>
                Buy {plan}
              </button>
            </div>
          ))}
        ...
        <div className="border-b border-gray-200">
          ...
          {securityOpen && (
            <div className="border-t border-gray-200">
              ...
              {/* Rest of the security features remain exactly the same */}
              {/* Scam Protection */}
              <div className="grid grid-cols-4">
                <div className="p-6 border-b border-gray-200">
                  <div className="flex items-start gap-3">
